...
with DAG(
        ...
        tags=['example', "spark", "python", "pyspark2"],
) as dag:
    kpo = KubernetesPodOperator(
        namespace='spark-sapp-work',
        task_id="ctemp-py2",
        name="ctemp-airflow-py2",
        ...
        cmds=["/bin/sh", "-c", """
            . /opt/confBuilder.sh
              ...
              set -x
              set -f
              /opt/spark/bin/spark-submit --master k8s://https://kubernetes.default.svc --deploy-mode client --name ctemp-airflow-py2 $CONF $PY_CODE \
              --src "s3a://spark-sapp/data/city_temperature.csv"  --bucket spark-sapp --database sapp --table ctemp_airflow_py2 --datamartFolder /warehouse/sapp.db \
